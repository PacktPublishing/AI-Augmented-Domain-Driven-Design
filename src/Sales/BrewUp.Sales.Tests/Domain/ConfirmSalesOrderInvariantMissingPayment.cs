...
public sealed class ConfirmSalesOrderInvariantMissingPayment : CommandSpecification<ConfirmSalesOrder>
{
    private readonly SalesOrderId _salesOrderId = new(Guid.CreateVersion7().ToString());
    private readonly SalesOrderNumber _salesOrderNumber = new("SO-2001");
    ...
    public ConfirmSalesOrderInvariantMissingPayment()
    {
    ...
    // Empty PaymentAuthorizationReference — invariant must block confirmation (BC-010)
    protected override ConfirmSalesOrder When() =>
        new(_salesOrderId, _correlationId, new PaymentAuthorizationReference(string.Empty), _stockRef);
    ...
    {
        // No SalesOrderConfirmed — invariant prevents confirmation without payment reference
        yield break;
    }
}
