...
public sealed class Chapter9RepositoryCatalog
{
    ...
    private static string ReadRequired(string repositoryRoot, string relativePath)
    {
        ...
        if (!File.Exists(path))
            throw new FileNotFoundException($"Required repository record is missing: {relativePath}", path);
        ...
        string relativePath)
    {
        ...
        if (!match.Success)
            throw new InvalidDataException($"{relativePath}: {label} SHA-256 is missing");
